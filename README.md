...
slides/
  ...
  jdnet-deck.html           JDネット サービス紹介（18枚・商材を持つ企業へ送る／明朝の雑誌型）
...
商材を持つ企業に**送って読ませる**ための18枚。編集UIと保存の仕組みはAIインサイトセールスの2デッキと同じで、**見た目だけが別系統**。
...
- **画像スロットは8つ**　表紙 `KV-01`／章扉 `CH-01`〜`CH-04`／締め `END-01`／ロゴ `LOGO-01`・`LOGO-02`。画像を入れるまではSVGの作図が出るので、空でも成立する
- **CTAは置かない**　送付して概要を理解してもらうのが役割。最終ページは締めのメッセージと会社情報のみ
- **載せないもの**　料金・掲載条件・申込フロー・「年間KPIにコミット」のような保証に読める表現
- **保存先は他の2デッキと別**（`jdnet-deck-v1`）

構成：01 表紙／02 章扉 課題／03 届く範囲は変わらない／04 4つの壁／05 章扉 仕組み／06 4つの販路／
07 パートナー網＝既存の信頼／08 約4万社から選ぶ／09 月1回の接点で育てる／10 章扉 進め方／
11 勝ち筋を見つける／12 年間で経路を組み替える／13 掲載の先＝共同で売る体制／14 章扉 実績／
15 基盤の数字／16 掲載企業の獲得数／17 Before / After／18 締め
