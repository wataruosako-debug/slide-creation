...
slides/
  ...
  jdnet-deck.html           JDネット サービス紹介（10枚・商材を持つ企業へ送る／明朝の雑誌型）
...
- **スマホは別の操作モデル**（iOSの contenteditable は選択が消える・入力が止まるなど不安定なため使わない）
  ...
  - **PC／スマホの切り替えは「指で操作する端末か」だけで判定する**（`pointer:coarse`）。画面幅では判定しない。
    窓を狭めただけのPCがスマホ用に切り替わってしまうため。判定が外れたときは、ツールバーの
    `操作を切替` で手動で変えられる（選択は端末に保存され、読み直しても残る）
  - JSの判定とCSSは必ず `body.touch-ui` で一致させる。ここを別々の条件（JSは幅、CSSはメディアクエリ）に
    すると、両方の条件が食い違う画面幅で**どちらのUIも出ない**状態が生まれる
...
## JDネット資料 `jdnet-deck.html`

商材を持つ企業に**送って読ませる**ための10枚。編集UIと保存の仕組みはAIインサイトセールスの2デッキと同じで、**見た目だけが別系統**。

- **意匠は雑誌型エディトリアル**　見出しは明朝（Noto Serif JP、なければ Hiragino Mincho / Yu Mincho へフォールバック）、面は白、角丸なし、罫線で揃える
- **色はロゴ由来の赤 `#C6202C` と濃紺 `#16233C` の2色だけ**。赤は現在地・強調・到達点にしか使わない
- **シグネチャ＝左端の章インデックス**　全ページの左端に「01 課題／02 仕組み／03 進め方／04 実績」を縦組みで通し、現在章だけ赤で示す（`.idx`、数字は `text-combine-upright` で縦中横）
- **図解はすべてコードで描く**（CSS/SVG）。文字が潰れず、あとから文言だけ直せる
- **1ページに余白を残さない**　各ページの本文ゾーンは `flex`／`grid` で縦に張らせる。図の高さを固定値で置かず、`flex:1` と `justify-content:space-between` で埋める
- **画像スロットは4つ**　表紙 `KV-01`／締め `END-01`／ロゴ `LOGO-01`・`LOGO-02`。画像を入れるまではSVGの作図が出るので、空でも成立する
- **CTAは置かない**　送付して概要を理解してもらうのが役割。最終ページは締めのメッセージ、Before/Afterの要約、会社情報
- **載せないもの**　料金・掲載条件・申込フロー・「年間KPIにコミット」のような保証に読める表現
- **保存先は他の2デッキと別**（`jdnet-deck-v1`）

構成：01 表紙／02 課題（届く範囲は変わらない＋4つの壁）／03 4つの販路／04 パートナー網＝既存の信頼／
05 約4万社から選ぶ／06 月1回の接点で育てる／07 勝ち筋の発見と年間サイクル／08 掲載の先＝共同で売る体制／
09 基盤の数字と掲載企業の獲得数／10 締め（Before/After＋会社情報）
